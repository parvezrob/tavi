# Tavi connection architecture — research memo v2 (2026-09-09)
...
What the field actually does (full survey in `01-product-survey.md`):
- **Every coding-agent product** (Claude Code Remote Control, Codex Remote, Cursor self-hosted machines, VS Code Tunnels, Happy Coder, Omnara) uses one long-lived **outbound** connection from the user's machine to a vendor relay, and sells "no inbound ports" as the security feature. **None hole-punches.** Only Happy Coder and VS Code Tunnels keep end-to-end encryption; Anthropic stores the plaintext transcript server-side and says outright that this is what makes cross-device reconnect work.
- **Remote-desktop products** (Parsec, Jump, Chrome Remote Desktop, RustDesk) hole-punch because video needs UDP throughput, and the ones without a relay (Parsec on CGNAT, Moonlight, Screens) have the worst "works at home, fails remotely" reputations. RustDesk merged WebRTC on 2026-09-05 because "ICE covers a strictly larger connectivity set" than its own punching.
- **Roaming without re-auth exists in exactly four designs**: Mosh (highest-sequence datagram rebinds the address), WireGuard (outer IP; whitepaper cites Mosh), QUIC (connection IDs), Chrome Remote Desktop (ICE restart, 10 s deadline, "without needing to re-authenticate"). Parsec, Sunshine and every plain WebSocket product drop on handover and re-dial.
- **Heartbeats converge on 3–5 s** (Mosh 3, Tailscale magicsock 3, Eternal Terminal 5): detect death fast, learn the new address, and hold NAT mappings that expire around 30 s. Our host heartbeat is 15 s × 2.
- **Control planes are the outages customers see as "your machine is offline"**: Jump Desktop 24 h (message-bus partition), Happy ~20 h, Tailscale Funnel dying silently after a control-plane reconnect while reporting healthy (#21114, 2026-09).
- **iOS background**: Blink, Termius and Panic all use Location "Always" to keep the process alive (20–30 s otherwise); Blink serializes mosh's SSP state to disk and rehydrates it on resume. Live Activities is the emerging successor.
- **Cellular-specific failures are IPv6/NAT64/CGNAT**: invisible on Wi-Fi, a large share of "fails on LTE" reports across products.

For a terminal at 1–5 KB/s the bandwidth argument for P2P evaporates; what matters is relay proximity, dead-link detection, and a session that survives the switch. That is the coding-agent class's design, done properly.
...
6. **Suspension is normal, not an error** (Blink's lesson): on background the phone snapshots the session state (epoch, offsets, unacked input, last frame) to disk within the background window; on foreground it resumes from that snapshot over whichever route answers first, before any UI redraw. Live Activities can extend the visible session later; Location "Always" is not acceptable for Tavi.
7. **Encryption end to end**: a Noise IK handshake keyed by the pairing (host static key from the QR fingerprint; device static key minted at pairing) inside every route, so the relay, Cloudflare, or a hostile Wi-Fi sees ciphertext only. Route TLS becomes defence in depth.
...
- The phone dials **every configured route in parallel** on every connect (Happy Eyeballs style), keeps the first that completes the Noise handshake and delivers a snapshot, keeps one alternate warm, and moves to it the moment the active one misses a heartbeat or the path changes (make-before-break on `betterPath`, as Apple documents).
- **Heartbeat 5 s while the terminal is on screen, two misses = dead (10 s)**, matching Mosh/Tailscale/ET rather than our 15 s × 2 = 45 s. The radio cost is measured, not assumed (Apple publishes no tail-time figure); the idle home screen keeps the slower cadence. A phone ping deadline is the only dead-peer detector that works behind CGNAT.
- Dual-stack hostnames only, never IP literals, so IPv6-only cellular (NAT64) works and App Review 2.5.5 is met.
...
- **No control plane.** The host's public key is its address on the relay; there is no directory, no account server, no presence database to partition. That is the lesson from Jump/Happy/Funnel: the data path was fine and the signalling died.
- Not Cloudflare Tunnel for the product path: TLS terminates at the edge (we would add Noise anyway), a public hostname per host on our zone, an unnumbered WebSocket idle timeout, WebSockets cut on Cloudflare code deploys, and cloudflared #1652 (open since 2026-05) drops the `Upgrade: websocket` header on its default QUIC transport. Fine for a one-day prototype of the phone side; not the thing we ship.
...
- **Cross-device state**: Anthropic keeps the transcript server-side to make reconnect work across devices; Tavi keeps it on the host, which is the user's own machine. Same benefit, no plaintext at a vendor.
...
5. Heartbeat 5 s on LTE: what does it cost in battery, and is 10 s to detect death good enough for the 1 s switch SLO when the alternate route is already warm?
6. What did we miss that Parsec/Jump/Tailscale/Blink learned the hard way?
