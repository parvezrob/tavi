...
- **One decision-maker per computer.** `AgentDirectory` owns "is this computer reachable": the health probe is single-flight (concurrent askers share the one in flight); the events stream redials at once on a drop (2 → 30 s jittered backoff, reset only after the stream has been up for 30 s, never on the first frame); the probe runs beside the redial, never in front of it; the repository poll pauses while the stream is down.
