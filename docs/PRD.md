...
- **One decision-maker per computer.** `AgentDirectory` owns "is this computer reachable": the health probe is single-flight (concurrent askers share the one in flight); the events stream redials at once on a drop (2 → 10 s jittered backoff, reset only after the stream has been up for 30 s or when the app comes to the foreground, never on the first frame; the handshake budget is 8 s); the probe runs beside the redial, never in front of it; the repository poll pauses while the stream is down.
- **Offline is earned, not guessed.** "Offline" needs two consecutive failed dials of the events stream *and* two missed probes. One drop is "Reconnecting" with the last state on screen; a frame clears it. A request that takes long shows a stale mark, never a frozen screen.
- **A socket is judged by its own heartbeat.** The events stream pings when idle for 30 s and is cycled after 45 s of silence (the host's WebSocket server answers pings). A terminal keeps its connection through a path *change*; it sends a heartbeat at once and lets the heartbeat's 5 s timeout decide. Only a path that is *unsatisfied* ends a connection, and a path change never resets the backoff.
...
The audit, the P1/P2 pass, and the direction correction (pure graphite, one amber) are on the issue; `TaviTheme.swift` is the one source of colour and radius. What the close-out fixed and keeps:
