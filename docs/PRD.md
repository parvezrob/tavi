...
- Host contract, written into `protocol/README.md` as each part lands: `GET /api/repos` (#59a), `POST /api/worktrees` (#75), `/api/worktrees/{status,stage,unstage,commit,commit-message}` (#77), `/api/worktrees/{log,push,pull-base}` (#78) — all shipped; still to come: `DELETE /api/worktrees/{id}`, PR create and link. *Pull main in* merges the **local** base branch (no fetch first); pushing never forces and never prompts.
