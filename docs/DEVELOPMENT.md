...
- `MOCHA_DEV_FONT_SIZE=<points>` seeds the terminal font preference at launch (#51). It exists because the preference lives in the app container, which `simctl spawn defaults` cannot reach and cfprefsd caches against direct plist edits. `MOCHA_DEV_RESET=1` clears the host, credential, pairing record, font size, viewport record, and the Face ID lock flag.
