...
## Feature freeze (owner decision, 2026-09-02)

The open issues on 2026-09-02 are **the whole remaining feature set**: #58 (dev-server preview, built; durability + phone memory checks left), #66 and #67 (Terminal pane detection hand-back; create opens the terminal), #59 (worktrees), #52 and #54 (home polish, design pass), #49 (WSL2 doc), #39 (test seam), #37 (App Store readiness epic), and the parked post-MVP trio #27–#29 (do or close, no growth). **After these, feature requests are frozen.** Work is then performance, optimization, and health only: memory and leaks on the phone (Ghostty surface, WKWebView, long-lived tasks), host soak and load, reconnect and offline behaviour, battery, startup time, test determinism, crash-free hours. A new capability needs the owner to lift the freeze explicitly; an agent does not file feature issues on its own. The post-freeze plan with baseline numbers and ranked findings is **#68** (performance) and **#69** (code health to 9/10).

**Freeze lifted for one area — source control (owner decision, 2026-09-02, ~17:20):** the bar for git integration is **Orca's mobile app, matched then beaten**, and this is allowed to go past #59's text ("if we can't match it and improve on it, might as well scrap the project"). Orca's mobile floor, verified from its docs and reference screenshots: one row per worktree (branch first, repo second, agent status on the row); create a worktree from the phone off a chosen base with task-source modes; changes list, stage/unstage, commit, create PR, link an existing PR; ahead/behind; commit log. Not on Orca's phone: remove worktree, push, pull — ours to own. What beats Orca is the join it lacks: agent status and git state on the same row. #59a (read-only branch/uncommitted/ahead-behind on the folder card, 49eb273 + 1768b2c) is below that floor and is the foundation, not the feature. The plan lives on the source-control epic issue; the rest of the freeze stands.
