...
- **Host runs the #24 build; the phone does not yet.** Install the new app build on the device before creating agents from it (required `cwd` is breaking — the #23 build gets a 400 on create; viewing, terminal, and permission decisions are unaffected).
