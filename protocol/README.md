...
### `GET /api/herdr/tree` · host 0.1.0

Herdr's own workspace → tab → agents hierarchy (#55, #67), for the Jump-to sheet. Read-only, composed from `workspace.list` + `tab.list` + the agent list, so every agent here carries the same identity `GET /api/agents` uses.

→ `{ "workspaces": [{ "workspaceId", "label", "focused", "tabs": [{ "tabId", "label", "focused", "agents": [Agent…] }…] }…] }`. `focused` is what herdr has in front on the Mac right now; a tab with no agent in it is still listed, and a workspace with no `workspace_id` is not. `404` when herdr is not configured on this host, `503` with herdr's sentence when it is but did not answer.
...
{ "repos": [{
  ...
  "name": "app",
  "defaultBranch": "main",
  "branches": ["main", "fix/foo"],
  "worktrees": [
    { "path": "/Users/me/Projects/app", "branch": "main", "head": "a1b2c3d4e5f60718293a4b5c6d7e8f9012345678", "isMain": true, "dirty": 0, "ahead": 0, "behind": 0, "locked": false, "prunable": false, "pullRequest": null, "withinRoots": true },
    { "path": "/Users/me/Projects/app-fix-foo", "branch": "fix/foo", "head": "e4f5a6b7c8d90112233445566778899aabbccdde", "isMain": false, "dirty": 2, "ahead": 3, "behind": 1, "locked": false, "prunable": false, "pullRequest": { "number": 48, "url": "https://github.com/me/app/pull/48" }, "withinRoots": true }
  ],
  "truncated": false
}], "truncated": false }
```

`branch` is `null` for a detached `HEAD`. `head` is the full SHA. `pullRequest` (#74) is the open pull request for the branch per the host user's own `gh` login (`gh pr list --head <branch>`), remembered for a minute per branch; `null` when there is none, when `gh` is not installed or not logged in, when the remote is not GitHub, for a detached worktree, or when the lookup did not answer within the call's 3 s budget (at most four `gh` at a time per repository; the rest are stopped, not left running — #85) — a client shows no badge and says nothing. `dirty` is a count of changed-or-untracked entries (`status --porcelain=v2`), not the files themselves — see `/api/changes` for those. When git could not read that worktree's status at all (it moved on disk, an index lock, a timeout), the row carries `"dirtyFailed": "<git's sentence>"` and `dirty` stays `0` — a client says the count is unknown rather than showing a clean worktree; the field is absent whenever it was counted (#103). `ahead`/`behind` are commits relative to `defaultBranch` (the remote's default branch, else a local `main` or `master`, else `null` and both `0`), measured as `refs/heads/<default>` or, with no local copy, `refs/remotes/origin/<default>`; a worktree already on the default branch reports `0`/`0`; a detached worktree is measured by its `head`. When git could not make the comparison at all (an index lock, a timeout, a default branch with no ref that resolves), the row carries `"aheadBehindFailed": "<git's sentence>"` and both counts stay `0` — a client labels the pair unknown rather than "in sync"; the field is absent whenever they were measured (#98). `defaultBranch` is `null` when none of those resolve. `withinRoots` (#83) says whether the worktree is inside the configured roots — the source-control routes below answer only there. Each repository's `truncated` is `true` when its worktrees (cap 40) or `branches` (cap 200) were cut; the top-level `truncated` when the repositories were (cap 40). When git itself is not installed the answer is `{ "repos": [], "truncated": false, "error": "git is not installed on this computer." }` — an empty list with no `error` means there are none.
...
When one of those files could not be copied — a permission error, an unreadable repository folder — the worktree is still created and the answer adds `"setupFilesFailed": "2 setup files could not be copied into the new worktree (EACCES)."` inside `worktree`, beside `copiedSetupFiles`: a count and the error code, never the names, and never a bare `"copiedSetupFiles": 0`, which is what a repository with no setup files looks like (#98). The field is absent when the copy ran clean.
...
- `GET /api/worktrees/status?path=…` → `{ "path", "branch", "base", "ahead", "behind", "files": [ChangedFile…], "staged", "truncated" }` — `/api/changes`' files plus the branch's ahead/behind against `base`: `branch.<b>.base` when set (Tavi sets it on create), else the repository's default branch; `null` and `0`/`0` when there is none. As on `/api/repos`, a comparison git could not make adds `"aheadBehindFailed": "<git's sentence>"` beside the two `0`s, and a client shows the pair as unknown (#98).
...
- `POST /api/worktrees/commit` `{ "path", "message" }` → `201 { "commit": { "sha", "summary", "files" } }` of exactly the staged set. `400` for an empty message; `409` with a sentence when nothing is staged or git has no identity on the computer; `503` with git's sentence when `diff --cached` itself failed, so a failed read is never presented as "Nothing is staged" (#98).
- `POST /api/worktrees/commit-message` `{ "path" }` → `{ "message" }`: one conventional-commit line for the staged diff, written by the `claude` CLI on the computer (`claude -p`, resolved on the login-shell PATH, 45 s), with secret-looking files left out of the diff by name (they are named, never shown). `409` when nothing is staged; `503` with a sentence when `diff --cached` failed (#98), or when claude is not installed or answered nothing — the phone offers to type one.
...
| `asOf` | number, optional · host unreleased | epoch ms of the herdr reply this list came from — present **only** while the host is serving a held list (see below) |

`asOf` is the one field that says a snapshot is not fresh. A host whose herdr misses a deadline no longer
erases the computer: it keeps serving the last list herdr actually gave — same `available: true`, same
`agents` — and stamps it with the moment it was read, while it retries. Absent means fresh, which is what
every snapshot before this field was, so a client that ignores `asOf` decodes exactly what it decoded
before and simply keeps showing the agents. A client that reads it can say how long ago the list was true
("last seen a moment ago"). Only when herdr has said nothing for the host's grace period (10 s) does a
frame arrive with `available: false`, an empty `agents`, and herdr's own sentence in `reason` — and a host
with no herdr at all still says that immediately, on the first frame, as it always did.
...
payload in its pong. Nothing on this socket is a delta, so a terminate costs a reconnect, never a
resynchronisation.
