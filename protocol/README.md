...
When one of those files could not be copied — a permission error, an unreadable repository folder — the worktree is still created and the answer adds `"setupFilesFailed": "2 setup files could not be copied into the new worktree (EACCES)."`: a count and the error code, never the names, and never a bare `"copiedSetupFiles": 0`, which is what a repository with no setup files looks like (#98). The field is absent when the copy ran clean.
...
- **Unreleased** (renamed to the version it ships in at the next publish) — no read path reports a number git did not produce (#98): `GET /api/repos` worktrees and `GET /api/worktrees/status` gained `aheadBehindFailed`, `GET /api/worktrees/pull-request` gained `unpushedFailed`, `POST /api/worktrees` gained `setupFilesFailed`, and `POST /api/worktrees/commit` / `commit-message` answer `503` rather than "Nothing is staged" when `diff --cached` failed. Every new field is additive and optional, and the counts it qualifies keep their old zeros, so a phone built against 0.1.17 decodes the answers unchanged.
