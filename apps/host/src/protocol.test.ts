...
import { chunkTerminalOutput, MAX_TERMINAL_FRAME_BYTES, parseClientTerminalMessage } from "./protocol.js";
...
test("terminal output chunks preserve Unicode and bound serialized frames", () => {
  ...
  for (const data of chunks) {
    assert.ok(Buffer.byteLength(JSON.stringify({ type: "output", data })) <= MAX_TERMINAL_FRAME_BYTES);
  }
});

async function loadFixture(): Promise<ProtocolFixture> {
  const path = fileURLToPath(new URL("../../../protocol/fixtures/terminal-v1/messages.json", import.meta.url));
