...
test("saveUpload writes the image under .tavi/uploads, keeps it out of git, and answers the path", async () => {
  ...
  const saved = await saveUpload({ cwd: project, roots: [root], contentType: "image/png", body: Buffer.from("png-bytes") });
  ...
  const lines = readFileSync(path.join(project, ".git", "info", "exclude"), "utf8").split("\n").filter((line) => line === ".tavi/uploads/");
...
test("saveUpload refuses what it must: outside the roots, not an image, empty, too large, a file for a cwd", async () => {
  ...
  assert.deepEqual(refused, { ok: false, status: 403, error: "That file is outside your project folders.", outsideRoots: true });
  assert.equal((await saveUpload({ cwd: project, roots: [root], contentType: "text/plain", body: Buffer.from("x") })).ok, false);
  assert.equal((await saveUpload({ cwd: project, roots: [root], contentType: "application/octet-stream", body: Buffer.from("x") })).ok, false);
  assert.equal((await saveUpload({ cwd: project, roots: [root], contentType: "image/png", body: Buffer.alloc(0) })).ok, false);
  const big = await saveUpload({ cwd: project, roots: [root], contentType: "image/png", body: Buffer.alloc(MAX_UPLOAD_BYTES + 1) });
  assert.equal(big.ok, false);
  if (!big.ok) assert.equal(big.status, 413);
  writeFileSync(path.join(project, "notes.txt"), "hi");
  const notAFolder = await saveUpload({ cwd: path.join(project, "notes.txt"), roots: [root], contentType: "image/png", body: Buffer.from("x") });
