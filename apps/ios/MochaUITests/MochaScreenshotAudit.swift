...
// MOCHA_AUDIT=1 so normal suite runs never pay for it. Run with the live
...
// them in teardown.
final class MochaScreenshotAudit: XCTestCase {
    ...
    func testCaptureAuditScreens() async throws {
        ...
        guard environment["MOCHA_AUDIT"] == "1" else {
            throw XCTSkip("Set TEST_RUNNER_MOCHA_AUDIT=1 to capture the design-audit screens.")
        }
        guard let host = environment["MOCHA_DEV_HOST"],
              let token = environment["MOCHA_DEV_TOKEN"] else {
            throw XCTSkip("Set TEST_RUNNER_MOCHA_DEV_HOST/TOKEN to run the audit against a live host.")
        }
