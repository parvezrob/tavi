...
// What the QR on the Mac carries (#45): `tavi://pair?u=<https url>&s=<secret>
...
struct PairingPayload: Equatable {
    ...
    enum DecodeError: Error, Equatable, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .notAPairingCode: "That is not a Tavi pairing code."
    ...
    static func decode(_ text: String) throws -> PairingPayload {
        guard let url = URL(string: text.trimmingCharacters(in: .whitespacesAndNewlines)),
              url.scheme?.lowercased() == "tavi", url.host?.lowercased() == "pair",
