...
// and token themselves are seeded by SessionsView from TAVI_DEV_HOST and
// TAVI_DEV_TOKEN, so the terminal always opens on the paired host.
struct TerminalDevelopmentBootstrap: Sendable {
    // TAVI_DEV_AGENT: the herdr pane whose terminal opens straight from
    // launch, without a tap on the home.
    let agentPaneID: String?
    // TAVI_DEV_RENDERER_STRESS_CHUNKS: a JSON array of output chunks the
