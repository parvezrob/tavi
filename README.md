...
The login service is macOS-only for now; on Linux run `npx tavi-host` in a terminal and keep it open (#48 adds a systemd unit). Linux also needs a C++ toolchain once (`sudo dnf install -y gcc-c++ make python3` on Fedora, `sudo apt install -y build-essential python3` on Debian/Ubuntu) because the terminal module is compiled at install time; `npx tavi-host doctor` says so if it is missing.
