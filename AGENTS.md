...
- **Connection code gets a fault-injection round (2026-09-06, #107):** any retry, heartbeat or watchdog state machine is not done until tests drive the real type with a slow peer, a send that never returns, and a close delivered mid-await on the ordering that wedges it — through injected transport and timing, never wall-clock sleeps. Structure and coverage scores said nothing about five recovery defects that this round found on the first try.
- **Cold GPT seat on plans and diffs (owner directive, 2026-09-06, #111):** connection or protocol work gets a `second-opinion` plan review before implementation and a `second-opinion` code review on the diff before its PR opens (`.claude/skills/second-opinion`, model `gpt-6-astra`). Round structure that converged: two seats with distinct lenses for rounds 1–2, then fold-audit seats only, then the owner locks — rounds 3+ returned wording, rounds 1–2 returned product defects.
- **Sub-agents (owner decision, 2026-09-03):** an orchestrating session may hand issue-scoped packages to Opus 5 sub-agents in isolated worktrees, one package per non-overlapping set of files, with a strict brief (scope, acceptance, "no new abstractions, comments only for why"). The loop (owner's, 2026-09-03): the implementer reports done → **two cold-context Opus 5 verifiers** read the issue and the diff independently (one for correctness and scope — exactly the acceptance, no behaviour change; one for quality — over-engineering, narrating comments, style drift) → findings go back to the implementer, verifiers re-check → the orchestrator then reads the diff, runs the gates, and commits. A sub-agent's "done" is a claim, not a result. Sub-agents never commit to `main`. **Integrating a worktree (learned 2026-09-04):** overlay code files, but never overlay a shared doc (`docs/DEVELOPMENT.md`, `protocol/README.md`, `AGENTS.md`) — two packages' overlays clobbered each other's sections twice in one night; `git merge-file` the worktree's version against its base commit and `main`, and diff the result for every section a package claims to have written.
- **Fix what you find (owner directive, 2026-09-09):** a defect surfaced by a review, a verifier or a soak inside the package being worked on gets fixed in that package — widen the brief, not the issue tracker. Filing an issue is for a defect outside the package's files that cannot be reached without a new design; even then, say so to the owner first. The owner does not want to triage tickets that an agent could have fixed.
