...
- **Sub-agents (owner decision, 2026-09-03):** an orchestrating session may hand issue-scoped packages to Opus 5 sub-agents in isolated worktrees, one package per non-overlapping set of files, with a strict brief (scope, acceptance, "no new abstractions, comments only for why"). The orchestrator integrates: read the diff, run the gates, then commit — a sub-agent's "done" is a claim, not a result. Sub-agents never commit to `main`.
